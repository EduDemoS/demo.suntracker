...
#else
AccelStepper stepper(AccelStepper::FULL4WIRE, PIN_STEPPER_1, PIN_STEPPER_3, PIN_STEPPER_0, PIN_STEPPER_2);
#endif
...
bool connectToWiFi();
bool connectToMQTT();
void sendDataToMQTT(float* data, size_t length);
...
void setup()
{
  ...
  // Run Startup Checks:
    ...
    bool result = runDiagnostics(true, true, true, true, true);
...
{ 
  ...
  static long lastI2CScan = 0;
  if((millis() - lastI2CScan) >= 200)
  {
    checkI2CDevices();
    lastI2CScan = millis();
  }
  ...
  static long lastSensorUpdate = 0;
  if((millis() - lastSensorUpdate) >= SENS_UPDATE_PERIOD)
  {
  ...
  if((millis() - lastLCDUpdate) >= LCD_UPDATE_PERIOD)
  {
  ...
  if(!cancelWiFi)
  {
    static long lastMQTTUpdate = 0;
    if((millis() - lastMQTTUpdate) >= MQTT_UPDATE_PERIOD)
    {
      lastMQTTUpdate = millis();

      // Check if WiFi connection is good:
      ...
      // Check if MQTT connection is also good:
      if(!mqtt.connected())
      {
        connectToMQTT();
      }
      ...
      float transmitData[5];
      for(uint8_t i = 0; i < 4; i++)
      {
        transmitData[i] = float(sensorData.PCFReadings[i]);
      }
      transmitData[4] = sensorData.solarVoltage;

      // Send it to the server:
      sendDataToMQTT(transmitData, 5);
    }
  }
  #endif
...
bool connectToWiFi()
{
  ...
  // Generate pretty SSID char[]:
  char wifi_ssid[]        = WIFI_SSID;
  char wifi_ssid_16[] = "                ";
  for(uint8_t i = 0; i < sizeof(wifi_ssid); i++)
  {
    wifi_ssid_16[i] = wifi_ssid[i];
  }

  Serial.print("Connecting to WiFi network '");
  Serial.print(wifi_ssid);
  Serial.print("' .");
  printToLCD(" Conn. Wifi...  ", wifi_ssid_16);
  ...
  while((WiFi.status() != WL_CONNECTED) && (attempts < WIFI_MAX_ATTEMPTS))
  {
    ...
    // Abort if connection can't be established:
    ...
    switch(WiFi.status())
    {
      case WL_IDLE_STATUS    : msg = "NOT STARTET YET "; break;
      case WL_NO_SSID_AVAIL  : msg = "NETW. NOT FOUND "; break;
      case WL_CONNECT_FAILED : msg = "FAILED (SIGNAL) "; break;
      case WL_CONNECTION_LOST: msg = "CONNECTION LOST "; break;
      case WL_WRONG_PASSWORD : msg = "INVALID PASSWORD"; break;
      case WL_DISCONNECTED   : msg = "DISCONNECTED    "; break;
      default                : msg = "UNKNOWN ( " + String(WiFi.status()) + ")   "; break;
    }
  ...
  printToLCD("  Connected.   ", WIFI_SSID);
  return true;
}
#endif
...
bool connectToMQTT()
{
  ...
  if(WiFi.status() != WL_CONNECTED)
  {
    Serial.println("Failed. No WiFi.");
    printToLCD("  MQTT ERROR:   ", 
               "    No WiFi     ");
    delay(2000);
    return false;
  }
  ...
  uint8_t attempts = 0;
  while(!mqtt.connected() && (attempts < MQTT_MAX_ATTEMPTS))
  {
    attempts++;
    Serial.print(".");
    ...
    {
      Serial.println("Connected.");
      printToLCD("  Connected.   ", MQTT_SERVER);
      delay(2000);
      return true;
    }
  }
  ...
  switch(mqtt.state())
  {
    case -4:  errorMsg = "CONN. TIMEOUT   "; break;
    case -3:  errorMsg = "CONN. LOST      "; break;
    case -2:  errorMsg = "CONN. FAILED    "; break;
    ...
    case  5:  errorMsg = "UNAUTHARIZED    "; break;
  ...
  Serial.println(").");
  printToLCD("  MQTT ERROR:   ", errorMsg.c_str());
  delay(1000);
  return false;
}
#endif
...
#if ENABLE_WIFI
// Send Photoresistor values and Solar Panel voltage to the MQTT Broker / thingspeak.
void sendDataToMQTT(float* data, size_t length)
{
  String topic = "channels/" + String(MQTT_CHANNEL_ID) + "/publish";
  String message = "";

  for(uint i = 0; i < length; i++)
  {
    message += "&field" + String(i+1) + "=" + String(data[i], 2);
  }
  message.remove(0, 1);   // Remove first '&'
...
void determineMovement(SensorData* data)
{
  ...
  avgTop   = (data->PCFReadings[0] + data->PCFReadings[1]) / 2;
  avgBot   = (data->PCFReadings[2] + data->PCFReadings[3]) / 2;
  avgLeft  = (data->PCFReadings[0] + data->PCFReadings[2]) / 2;
  avgRight = (data->PCFReadings[1] + data->PCFReadings[3]) / 2;
  avgSum   = (data->PCFReadings[0] + data->PCFReadings[1] + data->PCFReadings[2] + data->PCFReadings[3]) / 4;
...
void moveMotors(SensorData* data)
{
  ...
  // Check if Stepper has rotated more than 160deg either way from zero:
  
  static int16_t stepperLimit = stepperData.stepsPerDegree * 160;
...
bool runDiagnostics(bool checkLCD, bool checkPCF, bool checkWiFi, bool checkMQTT, bool checkServo)
{
  ...
  if(checkWiFi)
  {
    ...
    switch(wifiStatus)
    {
      case WL_IDLE_STATUS    : msg = "NOT STARTET YET "; break;
      case WL_NO_SSID_AVAIL  : msg = "NETW. NOT FOUND "; break;
      case WL_CONNECTED      : msg = "Connected.      "; break;
      case WL_CONNECT_FAILED : msg = "FAILED (SIGNAL) "; break;
      case WL_CONNECTION_LOST: msg = "CONNECTION LOST "; break;
      case WL_WRONG_PASSWORD : msg = "INVALID PASSWORD"; break;
      case WL_DISCONNECTED   : msg = "DISCONNECTED    "; break;
    ...
    Serial.println(msg);
    successWiFi = (wifiStatus == WL_CONNECTED);
    if(successLCD)
    {
      if(successWiFi)
      {
  ...
  if(checkMQTT)
  {
    ...
    auto wifiStatus = WiFi.status();
    auto mqttStatus = mqtt.state();
    String msg;
    if(wifiStatus != WL_CONNECTED) {msg = "ERROR: NO WIFI";}
    else
    {
      switch(mqttStatus)
      {
        case -4:  msg = "CONN. TIMEOUT   "; break;
        case -3:  msg = "CONN. LOST      "; break;
        case -2:  msg = "CONN. FAILED    "; break;
        case -1:  msg = "DISCONNECTED    "; break;
        case  0:  msg = "SUCCESS: Connected."; successMQTT = true; break;
        case  1:  msg = "BAD PROTOCOL    "; break;
        case  2:  msg = "BAD CLIENT ID   "; break;
        case  3:  msg = "UNAVAILABLE     "; break;
        case  4:  msg = "BAD CREDENTIALS "; break;
        case  5:  msg = "UNAUTHARIZED    "; break;
